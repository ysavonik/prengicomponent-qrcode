...
class QrcodeObjectsExportSearch extends Model
{
    public $filters;
    ...
    public function search($params) {
        $query = QrCodefunction::getAllAccessibleObjectsQuery();
        ...
        if (!($this->load($params, '') && $this->validate())) {
            $filter_m_obj = (new Query())
                ...
                ->andWhere('{{%qrcode_export_filter}}.id_qrcode_type=1')
                ->all();
            ...
            $m_obj_list = array();
            $obj_list = array();
            foreach ($filter_m_obj as $m_obj) {
                if ($m_obj['id_main_object'] != NULL) {
                    array_push($m_obj_list, $m_obj['id_main_object']);
                }
            }
        ...
        elseif (!empty($this->filter_ch_main_obj) || !empty($this->filters)) {
            ...
            if (!empty($this->filter_ch_main_obj)) {
                foreach ($this->filter_ch_main_obj as $m_obj) {
                    $sql .= "('" . $id_user . "', '1', '" . HtmlPurifier::process($m_obj) . "', NULL, NULL, NULL) ,";
                }
            }
            if (!empty($this->filters)) {
                $query->andFilterWhere(['{{%objects}}.id_object' => $this->filters['ft_objects']]);
                foreach ($this->filters['ft_objects'] as $obj) {
                    $sql .= "('" . $id_user . "', '1', NULL, '" . HtmlPurifier::process($obj) . "', NULL, NULL) ,";
                }
            }
//            if (isset($params['ft-select-objects'])) {
//                if (!empty($params['ft-select-objects'])) {
//                    $sql .= "('" . $id_user . "', '1', NULL, NULL, '" . HtmlPurifier::process($params['ft-select-objects']) . "', NULL) ,";
//                }
//            }
            YII::$app->db->createCommand("INSERT INTO {{%qrcode_export_filter}}
                            (`user_id`, `id_qrcode_type`, `id_main_object`, `id_object`, `object_str`, `id_category`) VALUES" . substr($sql, 0, -1))
                ->execute();
            $activeDataProvider = new ActiveDataProvider([
                'query' => $query,
                'pagination' => [
    ...
    public function clearFilters($id_user)
    {
        $command = YII::$app->db->createCommand("DELETE FROM  {{%qrcode_export_filter}}
                WHERE {{%qrcode_export_filter}}.user_id=" . $id_user . " and {{%qrcode_export_filter}}.id_qrcode_type=1");
        $command->execute();
    }
